...
> **Update (2026-06-24):** multi-month and annual `TimePeriod` windows are now supported for all six analyses that accept a `TimePeriod`: `solar-radiation`, `daylight-availability`, `direct-sun-hours`, `thermal-comfort-index`, `thermal-comfort-statistics`, and `pedestrian-wind-comfort`. A full-year wind rose is the standard way to assess pedestrian comfort. `wind-speed` and `sky-view-factors` take no `TimePeriod`.
>
...
**Building occlusion**: cells inside building footprints come back as NaN from the SDK (cell-level point-in-polygon clipping happens server-side), so the heatmap naturally has a "hole" under every building. That holds on the flat path; on a terrain-draped run (`ground_geometry` passed) footprint cells are `0.0`, not NaN — mask with your own footprints; see [`../interpretation/grid-conventions.md`](../interpretation/grid-conventions.md). The 3D extruded buildings then sit in that hole — no rendering conflict, no need to special-case daylight / SVF. If you ever see heatmap colour bleeding *through* a building, the polygon filter on the building layer is off (most often: filtering by first-vertex of the ring vs. the centroid).
...
3. **Multi-month TimePeriod** is now supported for all six analyses that accept a `TimePeriod`: `solar-radiation`, `thermal-comfort-index`, `thermal-comfort-statistics`, `daylight-availability`, `direct-sun-hours`, and `pedestrian-wind-comfort` (as of 2026-06-24). A full-year wind rose is the standard input for PWC. `wind-speed` and `sky-view-factors` take no `TimePeriod`.
...
- **Multi-month / annual TimePeriod for PWC.** `pedestrian-wind-comfort` supports multi-month and annual windows natively on prod (verified 2026-06-24). Pass the full wind-rose period directly — no client-side looping needed. All six TimePeriod-based analyses (solar-radiation, UTCI, TCS, DA, DSH, PWC) support this.
