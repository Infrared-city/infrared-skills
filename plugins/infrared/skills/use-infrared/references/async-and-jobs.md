...
For CI smoke scripts, non-Python clients and debugging. Paths from `analyses/jobs.py`:

| | |
|---|---|
| submit | `POST {base}/async/{analysis_type}` → `202 {jobId}` |
| poll | `GET {base}/async/jobs/{jobId}` — **not** `/jobs/{jobId}`, which 404s |
