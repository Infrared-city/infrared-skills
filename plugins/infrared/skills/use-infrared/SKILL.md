...
- TCS subtype is per-call — run three jobs for `thermal-comfort` + `heat-stress` + `cold-stress`.
- Verifying webhooks against re-encoded JSON instead of raw bytes (see [06-webhooks.md](references/06-webhooks.md)).

Runnable recipes: [`cookbook/`](https://github.com/Infrared-city/infrared-skills/tree/main/cookbook) (wind, UTCI Munich, all-8 multi-analysis, fetch-once-reuse, area tiling, lower-level primitives, async webhooks).
