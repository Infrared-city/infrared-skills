...
These notebooks demonstrate the additive **advanced** simulation inputs on
the raw wire. NOTE: since infrared-sdk 0.4.12 the facade/sensor/terrain/
context fields below are natively typed on the SDK models — only
``partial-cells``/``min-coverage``/``physics:"advanced"``/
``canopy-transmissivity`` remain direct-API-only:
