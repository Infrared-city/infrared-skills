...
def generate_terrain(
    *,
    ...
    # numpy's apply_along_axis stub mistypes func1d as zero-arg; the call is
    # correct -- silence the stub-only false positive, not a real error.
    for _ in range(3):  # separable smoothing passes
        noise = np.apply_along_axis(lambda m: np.convolve(m, k, "same"), 0, noise)  # pyright: ignore[reportCallIssue, reportArgumentType]
        noise = np.apply_along_axis(lambda m: np.convolve(m, k, "same"), 1, noise)  # pyright: ignore[reportCallIssue, reportArgumentType]
    z += 1.2 * noise
