...
#: The SDK analysis-type strings that every area must complete.
#: The ``-morning`` suffix is a demo-only DB key for a second UTCI run at a
#: different time of day — the SDK analysis type is still
#: ``thermal-comfort-index`` for both schedules; only the weather window
#: differs. We key the DB row separately so both can coexist under the same
#: area (the schema has ``UNIQUE(area_name, analysis_type)``).
...
FRIENDLY_NAMES: dict[str, str] = {
    "wind-speed": "wind",
    "thermal-comfort-index": "utci 2pm",
    "thermal-comfort-index-morning": "utci 9am",
}
...
def save_schedule(
    ...
    ----------
    analysis_type_key:
        Optional override for the ``analysis_type`` column used as the
        DB uniqueness key. Defaults to ``schedule.analysis_type``.
        Useful when the demo runs the same SDK analysis type twice
        (e.g. UTCI at 09:00 vs 14:00) and needs to distinguish the
        rows.

    Returns the ``area_run_id`` of the inserted/replaced row.
    """
    schedule_json = json.dumps(schedule.to_dict())
    analysis_type = analysis_type_key or schedule.analysis_type
