...
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "outputs", "wind_direction_sweep.html")
...
def main() -> None:
    with InfraredClient(logger=logger) as client:
        ...
        for direction in WIND_DIRECTIONS:
            ...
            logger.info(
                ...
                result.grid_shape, result.min_legend, result.max_legend,
            )

        logger.info(
            "Layers fetched 1x, reused across %d analysis runs", len(WIND_DIRECTIONS),
        )
    ...
    for idx, (direction, r) in enumerate(results):
        ...
        fig.add_trace(
            go.Heatmap(
                z=r.merged_grid, colorscale="Turbo",
    ...
    fig.update_layout(
        ...
        width=1200, height=1200, template="plotly_white",
    )
