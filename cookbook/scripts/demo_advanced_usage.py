...
def _custom_poll(client: InfraredClient, job_id: str, timeout: int = 120) -> Job:
    ...
    while True:
        ...
        attempt += 1
        logger.info(
            "  Poll #%d: status=%s, sleeping %.1fs...", attempt, job.status, delay
        )
        time.sleep(delay)
...
def example_area_composable(client: InfraredClient) -> AreaResult:
    ...
    if schedule.failed_submissions:
        logger.info(
            "Retrying %d failed submissions...", len(schedule.failed_submissions)
        )
...
def example_manual_pipeline(client: InfraredClient) -> np.ndarray:
    ...
    logger.info(
        "Generated %dx%d grid, %d non-empty tiles",
        tile_grid.num_rows,
        tile_grid.num_cols,
        len(non_empty),
    )
...
def example_byo_weather(client: InfraredClient):
    ...
    tp = TimePeriod(
        start_month=7, start_day=1, start_hour=14, end_month=7, end_day=30, end_hour=14
    )
    sdk_weather = client.weather.filter_weather_data(
    ...
    logger.info(
        "Built UTCI payload with %d custom weather points: %s",
        len(custom_weather),
        utci_payload.analysis_type,
    )
...
def example_webhook_workflow(client: InfraredClient):
    ...
    logger.info(
        "Submitted %d jobs with webhook_url=%s", len(schedule.jobs), WEBHOOK_URL
    )
